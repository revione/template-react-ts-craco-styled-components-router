;(() => {
  const getReaderTexts = () => {
    ...
    const forEachDl = (dl) => {
      ...
      // get dls
      const as = Array.from(
        dl.querySelector("dt").querySelectorAll("a").values()
      )
    ...
    const navigationFunction = (translationUsedInFor) => {
      ...
      const dls = Array.from(
        translationUsedInFor.querySelectorAll("dl").values()
      )
  ...
  const getTexts = () => {
    ...
    const forEachDl = (dl) => {
      ...
      // get dt and dd
      const asDt = makeArray(
        dl.querySelector("dt")?.querySelectorAll("a") || []
      )
      const asDD = makeArray(
        dl.querySelector("dd")?.querySelectorAll("a") || []
      )
